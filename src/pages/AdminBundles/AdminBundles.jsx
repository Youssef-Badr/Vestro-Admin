...
relatedProduct: [],
  ...
  if (fetchLoading) return (
    <div className="flex h-screen items-center justify-center bg-white dark:bg-gray-950">
      <RefreshCcw className="animate-spin text-red-500/80" size={50} />
    </div>
  );
    ...
    <div className={`p-4 md:p-10 min-h-screen bg-[#FDFDFD] dark:bg-gray-950 transition-colors duration-500 ${isRTL ? "font-cairo" : ""}`} dir={isRTL ? "rtl" : "ltr"}>
      ...
      <div className="flex flex-col mt-16 md:flex-row justify-between items-start md:items-center gap-6 mb-16">
        ...
        <motion.button
          ...
          className="w-full md:w-auto bg-red-700 dark:bg-red-800 text-white px-12 py-5 rounded-[2rem] font-black shadow-xl shadow-red-700/10 transition-all flex items-center justify-center gap-3 border border-red-700/30"
        >
          <Plus size={22} /> 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
        {bundles.map((bundle, index) => (
          <motion.div 
            ...
            key={bundle._id}
            className="bg-white dark:bg-gray-700/40 rounded-[3rem] border border-slate-100 dark:border-gray-800/50 p-7 shadow-sm hover:shadow-2xl hover:border-red-100 dark:hover:border-red-700/30 transition-all relative overflow-hidden group backdrop-blur-sm"
          >
            {/* Soft Status Badge */}
            ...
            <div className="flex items-center justify-between mb-8 px-2">
               <div className="flex flex-col">
                  <span className="text-3xl font-black text-red-700 dark:text-red-400">{bundle.bundlePrice} <small className="text-[10px] tracking-tighter uppercase opacity-60">EGP</small></span>
...
{bundle.relatedProduct && bundle.relatedProduct.length > 0 && (
  <div className="bg-slate-50 dark:bg-gray-800 p-3.5 rounded-2xl border border-slate-100 dark:border-gray-700 shadow-sm text-red-500 flex items-center gap-2">
    <LinkIcon size={20} />
               ...
               <div className="flex -space-x-4 rtl:space-x-reverse">
                 {bundle.items?.map((item, idx) => (
                   <img 
                      ...
                      className="w-14 h-14 rounded-full border-[5px] border-white dark:border-gray-700 object-cover shadow-md"
                      alt="v"
                   />
                 ))}
               </div>
  ...
  setShowModal(true);
                ...
                className="flex-[2.5] flex items-center justify-center gap-2 py-5 bg-red-700 dark:bg-red-800 text-white rounded-[1.8rem] font-black hover:bg-red-950 transition-all shadow-lg shadow-red-700/10"
              >
              ...
              <button 
                ...
                className="flex-1 flex items-center justify-center py-5 text-rose-500 bg-rose-50 dark:bg-rose-700/10 rounded-[1.8rem] hover:bg-rose-500 hover:text-white transition-all border border-rose-100 dark:border-rose-700/20"
              >
                <Trash2 size={22} />
              </button>
            </div>
      ...
      <AnimatePresence>
        {showModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-700/80 backdrop-blur-md">
            <motion.div 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  <div className="space-y-4">
                    ...
                    <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} 
                           className="w-full bg-slate-50 dark:bg-gray-700/50 dark:text-white p-7 rounded-[2rem] border-2 border-transparent focus:border-red-700/20 dark:focus:border-red-500/20 focus:bg-white outline-none transition-all font-bold text-lg" 
                  ...
                  <div className="space-y-4">
                    ...
                    <input required type="number" value={formData.bundlePrice} onChange={e => setFormData({...formData, bundlePrice: e.target.value})} 
                           className="w-full bg-slate-50 dark:bg-gray-700/50 dark:text-red-400 p-7 rounded-[2rem] border-2 border-transparent focus:border-red-700/20 outline-none transition-all font-black text-2xl text-red-700" 
                           placeholder="0.00" />
                  </div>
                </div>
...
<div className="space-y-4">
  ...
  {/* 🔥 GRID بدل الزحمة */}
  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-[220px] overflow-y-auto p-3 bg-slate-50 dark:bg-gray-700/50 rounded-2xl border">
        ...
        {isRTL ? "لا توجد منتجات" : "No products available"}
                ...
                <div className="space-y-6">
                   <div className="flex justify-between items-end px-3">
                     ...
                     <span className="text-[10px] font-black text-white bg-red-700 dark:bg-red-600 px-5 py-2 rounded-full shadow-lg uppercase tracking-widest">{formData.items.length} {isRTL ? "محدد" : "Selected"}</span>
                   </div>
                   <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-5 max-h-[480px] overflow-y-auto p-6 bg-slate-50 dark:bg-gray-700/30 rounded-[3rem] border-2 border-dashed border-slate-200 dark:border-gray-800 custom-scrollbar">
  ...
  className={`relative cursor-pointer group rounded-2xl overflow-hidden border-2 transition-all duration-500 shadow-sm ${
    isSelected
      ? 'border-red-700 dark:border-red-500 bg-white dark:bg-gray-800'
...
>
  ...
  <div className="p-2">
    ...
    <p className="text-[10px] text-red-700 dark:text-red-400 font-black mt-1">
      {product.salePrice || product.price} EGP
    </p>
  </div>

  {isSelected && (
    <div className="absolute top-2 right-2 bg-red-700 dark:bg-red-500 text-white rounded-full p-1.5 shadow-xl">
      <CheckCircle2 size={12} />
                ...
                <div className="flex flex-col lg:flex-row gap-10 items-center bg-slate-50 dark:bg-gray-700/50 p-10 rounded-[3.5rem] border border-slate-100 dark:border-gray-800/50">
                  <div className="flex-1 w-full">
                    <label className="block text-center border-4 border-dashed border-slate-200 dark:border-gray-800 rounded-[3rem] p-14 cursor-pointer hover:border-red-700/20 hover:bg-white dark:hover:bg-gray-700 transition-all group relative overflow-hidden">
                       <ImageIcon className="mx-auto text-slate-300 group-hover:text-red-700 dark:group-hover:text-red-500 mb-5 transition-colors" size={55} />
                  ...
                  {imagePreview && (
                    <div className="relative group">
                      <img src={imagePreview} className="w-64 h-64 rounded-[3.5rem] object-cover shadow-3xl border-[12px] border-white dark:border-gray-700 ring-1 ring-slate-100 dark:ring-gray-800" />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                    <label className="flex items-center justify-between p-8 bg-slate-50 dark:bg-gray-700/50 rounded-[2.5rem] cursor-pointer border border-transparent hover:border-red-700/10 transition-all group">
                        ...
                        <input type="checkbox" checked={formData.showInNavbar} onChange={e => setFormData({...formData, showInNavbar: e.target.checked})} className="w-7 h-7 accent-red-700 dark:accent-red-500" />
                    </label>
                    <label className="flex items-center justify-between p-8 bg-slate-50 dark:bg-gray-700/50 rounded-[2.5rem] cursor-pointer border border-transparent hover:border-emerald-500/10 transition-all group">
                ...
                <motion.button
                  ...
                  className="w-full bg-red-950 dark:bg-red-700 text-white py-9 rounded-[3rem] font-black text-2xl shadow-3xl shadow-red-950/20 transition-all disabled:bg-slate-300 flex items-center justify-center gap-5 uppercase tracking-widest"
                >
